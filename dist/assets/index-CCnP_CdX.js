 ...
 */Ze(["pointerdown","pointerup","pointermove","click","mousemove"]);const Vr=(t,e)=>{const n=function(){const r=localStorage.getItem(t);return r!=null&&r!=="undefined"?JSON.parse(r):e}(),[i,a]=F(n);return rt(()=>{localStorage.setItem(t,JSON.stringify(i))},[i]),[i,a]};var Xr=D('<div class="flex flex-col gap-4 pb-10"><div class="p-4 flex flex-col md:items-center "><div class="flex flex-col border border-b-0 bg-white w-fit"><div class="border-b justify-between p-2 text-lg font-bold flex"><h1>我是Galgame糕手<span class=remove> - 点击选择你玩过的Galgame</span><span class="ml-2 text-zinc-400 font-medium">galgame.fallenwood.net</span></h1><span class="shrink-0 whitespace-nowrap">我玩过 <!>/<!> 部Galgame</span></div></div></div><div class="flex gap-2 justify-center"><button type=button class="border rounded-md px-4 py-2 inline-flex">全选</button><button type=button class="border rounded-md px-4 py-2 inline-flex">复制图片</button><button type=button class="border rounded-md px-4 py-2 inline-flex">下载图片</button></div><div class="mt-2 text-center">历年关注最多的Galgame和部分RPG，数据来自 bgm.tv，<a href=https://github.com/fallenwood/galgame-sedai target=_blank class=underline>查看代码'),qr=D('<div class="flex border-b"><div class="bg-red-500 shrink-0 text-white flex items-center font-bold justify-center p-1 size-16 md:size-20 border-black"></div><div class="flex shrink-0">'),Gr=D('<button><span class="leading-tight w-full line-clamp-3"><img class="size-10 md:size-12 ml-auto mr-auto"></span><span class="leading-tight w-full line-clamp-3">'),Zr=D('<button type=button class="border rounded-md px-4 py-2 inline-flex">清除');const Qr=()=>{const[t,e]=Vr("selectedAnime",[]);let n;const i=async()=>(console.log("wrapper",n),n?await rr(n,{scale:2,filter(s){return!(s instanceof HTMLElement&&s.classList.contains("remove"))}}):void 0),a=async()=>{const o=await i();console.log(o),o&&await navigator.clipboard.write([new ClipboardItem({[o.type]:o})])},r=async()=>{if(!n)return;const o=await i();if(!o)return;const s=URL.createObjectURL(o),g=document.createElement("a");g.href=s,g.download="galgame-sedai.png",g.click(),URL.revokeObjectURL(s)};return(()=>{var o=Xr(),s=o.firstChild,g=s.firstChild,u=g.firstChild,l=u.firstChild,c=l.nextSibling,m=c.firstChild,f=m.nextSibling,_=f.nextSibling,h=_.nextSibling;h.nextSibling;var b=s.nextSibling,j=b.firstChild,w=j.nextSibling,x=w.nextSibling,y=n;return typeof y=="function"?ue(y,g):n=g,R(c,()=>t().length,f),R(c,()=>Object.values(Rt).flatMap(p=>p.map(E=>E.title).slice(0,12)).length,h),R(g,()=>Object.keys(Rt).map(p=>{const E=Rt[p]||[];return(()=>{var O=qr(),Y=O.firstChild,N=Y.nextSibling;return R(Y,p),R(N,()=>E.slice(0,12).map(G=>{const k=t().includes(G.title);return(()=>{var z=Gr(),T=z.firstChild,st=T.firstChild,nt=T.nextSibling;return z.$$click=()=>{e(Z=>k?Z.filter(K=>K!==G.title):[...Z,G.title])},lt(z,`size-16 md:size-20 border-l break-all text-center shrink-0 inline-flex flex-col items-center p-1 overflow-hidden justify-center cursor-pointer text-sm  ${k?"bg-green-500":"hover:bg-zinc-100"}`),R(nt,()=>G.title),X(Z=>{var K=G.title,ht=`/images/${G.img}`;return K!==Z.e&&C(z,"title",Z.e=K),ht!==Z.t&&C(st,"src",Z.t=ht),Z},{e:void 0,t:void 0}),z})()})),O})()}),null),j.$$click=()=>{e(Object.values(Rt).flatMap(p=>p.map(E=>E.title).slice(0,12)))},R(b,(()=>{var p=ae(()=>t.length>0);return()=>p()&&(()=>{var E=Zr();return E.$$click=()=>{e([])},E})()})(),w),w.$$click=()=>{console.log("copy image"),Fe.promise(a(),{success:"复制成功",loading:"复制中",error(p){return`复制失败: ${p instanceof Error?p.message:"未知错误"}`}})},x.$$click=()=>{Fe.promise(r(),{success:"下载成功",loading:"下载中",error(p){return`下载失败: ${p instanceof Error?p.message:"未知错误"}`}})},o})()};Ze(["click"]);const Jr=document.getElementById("root");Hn(()=>[q(Qr,{}),q(Kr,{})],Jr);
